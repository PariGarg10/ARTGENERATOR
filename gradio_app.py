...
import gradio as gr
from inference.generate import generate, load_pipeline
...
def ensure_pipeline(lora_path: str):
    global pipe
...
def run_generation(prompt: str, negative_prompt: str, lora_path: str, trigger_token: str):
    pipeline = ensure_pipeline(lora_path)
    image = generate(
        pipeline,
        user_prompt=prompt,
...
with gr.Blocks(title="Minimal FLUX LoRA Generator") as demo:
    ...
    generate_button.click(
        fn=run_generation,
        inputs=[prompt, negative_prompt, lora_path, trigger_token],
